...
class Asteroid(Center):
    ...
    def generateShape(self):
        ...
        if (a % 2 == 0):
            ...
            self.__asteroid = Rectangle(Point(self.getX(), self.getY()), Point(self.getX() + 50, self.getY() + 15))  # rec object of the asteroid
            self._isCircle = False
    ...
    def collision(self, originPoint):
        ...
        if self._isCircle == True:
            if Circle.testCollision_CircleVsPoint(self.__asteroid, originPoint):
                return True
            #if Circle.testCollision_CircleVsRectangle(self.__asteroid, originPoint):
                #return True
        elif self._isCircle == False:
            if Rectangle.testCollision_RectVsPoint(self.__asteroid, originPoint):
                return True
            #if Rectangle.testCollision_RectangleVsRe
        return False
    ...
    def isHit(self, missileList: list):
        ...
        while listIndex < len(missileList):
            if self._isCircle == True:
                ...
                if Rectangle.testCollision_RectangleVsCircle(self.__asteroid, missileList[listIndex].getSelf()):
                    self.__asteroid.undraw()
                    missileList[listIndex].getSelf().undraw()
                    missileList.pop(listIndex)
                    return True
...
class Missile(object):
    ...
    def __init__(self, rocket: Rocket):
        ...
        self.__size = 150    # Radius of the circle is small to simulate missile/bomb
...
class Message(object):
    ...
    title = Text(Point(375, 375), "Asteroids!!!\nPress 'Up' to play\nOr 'Down' to exit")
    title.setSize(36)
    title.setStyle("bold")
    title.setTextColor("White")

    lose = Text(Point(375, 375), "GAME OVER!")
    lose.setSize(36)
    lose.setStyle("bold")
    lose.setTextColor("Red")

    winMessage = Text(Point(375, 375), "YOU WIN!")
    winMessage.setSize(36)
    winMessage.setStyle("bold")
    winMessage.setTextColor("BLUE")

    levelMessage = Text(Point(375, 375), "Next Level!")
    levelMessage.setSize(36)
    levelMessage.setStyle("bold")
    levelMessage.setTextColor("BLUE")
...
def main():
    win = GraphWin("Asteroids", 750, 750, autoflush = False)
    ...
    message = Text(Point(375, 375), "Test Message")
    message.setSize(36)
    message.setStyle("bold")
    message.setTextColor("White")

    exitMsg = Text(Point(375, 375), "Mouse click to exit")
    exitMsg.setSize(36)
    ...
    while exit == False:
        ...
        if len(asteroidList) == 0:
            for i in range(random.randint(5,15)):  # random.randint(50,100)
                asteroidList.append((Asteroid(random.randrange(50, 750), random.randrange(50, 750))))
        ...
        if 'Return' in keyboard or 'KP_Enter' in keyboard:
            ...
            origin = Center(375, 375)  # Create a point to move the rocket on screen
            rocket = Rocket(375, 375, math.pi / 2)  # (x,y) starting point and pi/2 (90 degrees) starting direction
            ...
            while play == True:
                ...
                if len(ufoList) == 0 and len(asteroidList) == 0:
                    ...
                    if level < 6:
                        Message.levelMessage.draw(win)
                        print(f"Next Level")  # Print 'You Win' to console
                        for i in range(5):  
                            asteroidList.append((Asteroid(random.randrange(50, 850), random.randrange(50, 850))))
                            asteroidList[i].drawAsteroid(win)  # Draws the asteroid to the window
                        ...
                        print(f"Win")  # Print 'You Win' to console
